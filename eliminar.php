...
$id =$_GET['id'];
		...
		if($tipo == 'ingreso'){
			$qexiste = mysql_query("SELECT id_lote,idIngreso,iDeposito,idEntrega,idProceso FROM lotes l LEFT JOIN depositos d ON d.idLote = l.id_lote LEFT JOIN entregas e ON e.idLote = id_lote LEFT JOIN procesos p ON p.idLote = id_lote LEFT JOIN ingresos i ON i.idLote = id_lote WHERE idIngreso = $id",$link) or die(mysql_error());
			$existe = mysql_fetch_array($qexiste);
			if(($existe['idProceso'] == null)&&($existe['idEntrega'] == null)&&($existe['iDeposito'] == null)){
				$queEmp = "DELETE FROM ingresos WHERE idLote='$id'";
				$resEmp = mysql_query($queEmp, $link) or die(mysql_error());
				$queEmp = "DELETE FROM lotes WHERE id_lote='$id'";
				$resEmp = mysql_query($queEmp, $link) or die(mysql_error());
			}else{
				echo "Debe eliminar todos los procesos antes de eliminar ingreso y el lote";
			}
		}
