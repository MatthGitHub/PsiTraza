<?php
include('config.php');
    // Primero comprobamos que ning�n campo est� vac�o y que todos los campos existan.
    if(isset($_POST['idTipoProceso']) && !empty($_POST['idTipoProceso']) &&
    isset($_POST['cantidad']) && !empty($_POST['cantidad'])){
        // Si entramos es que todo se ha realizado correctamente
		$idLote = $_POST['idLote'];
    $tipo = $_POST['idTipoProceso'];
    $cantidad = $_POST['cantidad'];


    // Establecer la zona horaria predeterminada a usar. Disponible desde PHP 5.1
    date_default_timezone_set('UTC');
    //Imprimimos la fecha actual dandole un formato
    $fecha = date("Y-m-d");
    $anio = date("Y")+1;
    $vencimiento = date("$anio-m-d");

        $link = mysql_connect ($dbhost, $dbusername, $dbuserpass);
        mysql_select_db($dbname,$link);

		$queEmp = "SELECT idLote,tipoProceso FROM depositos WHERE idLote='$idLote' AND tipoProceso='$tipo'";
		$resEmp = mysql_query($queEmp, $link) or die(mysql_error());
		$totEmp = mysql_num_rows($resEmp);
		if($totEmp > 0){
		echo "Este lote ya paso por ese proceso!";
		exit();
		}

        // Con esta sentencia SQL insertaremos los datos en la base de datos
        mysql_query("INSERT INTO depositos (idLote,tipoProceso,fecha,vencimiento,cantidad) VALUES ('{$idLote}','{$tipo}','{$fecha}','{$vencimiento}','{$cantidad}')",$link);
        // Ahora comprobaremos que todo ha ido correctamente
        $my_error = mysql_error($link);

        if(!empty($my_error)) {

            header ("Location: form_deposito.php?errordat&numLote=<?php echo $idLote ?> ");

        } else {

             header ("Location: seguimiento_lote.php?idLote={$idLote}");

        }

    } else {

         header ("Location: form_deposito.php?errordb&numLote=<?php echo $idLote ?>");

    }

?>
